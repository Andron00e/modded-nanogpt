...
class Neon(torch.optim.Optimizer):
    """
    Neon: NEsterov momentum Orthogonalized by Newton-schulz iteration

    Let G_t be the update at timestep t generated by SGD with Nesterov momentum. Then Neon updates
    the weights using an efficient approximation of UV^T, where USV^T = G_t is the SVD of G_t.
    If momentum is turned off, then this target update UV^T is equivalent to the update produced
    by Shampoo (https://arxiv.org/abs/1802.09568) with no accumulation.
...
optimizer2 = Neon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=0.95)
